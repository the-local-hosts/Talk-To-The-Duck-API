...
const commentOwnership = (requestObject, resource) => {
  // `requestObject.user` will be defined in any route that uses `requireToken`
  // `requireToken` MUST be passed to the route as a second argument
  if (resource) {
    if (!requestObject.user._id.equals(resource.postedBy)) {
      throw new OwnershipError()
    }
  } else {
...
const likeOwnership = (requestObject, resource) => {
  // `requestObject.user` will be defined in any route that uses `requireToken`
  // `requireToken` MUST be passed to the route as a second argument
  if (resource) {
    if (!requestObject.user._id.equals(resource.likedBy)) {
      throw new OwnershipError()
    }
  } else {
    throw new DocumentNotFoundError()
